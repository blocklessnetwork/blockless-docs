...
export function Search({
	...
	return (
		<div className={cn('nextra-search nx-relative md:nx-w-64', className)}>
			{renderList && (
				<div className="nx-fixed nx-inset-0 nx-z-10 nx-h-[100vh]" onClick={() => setShow(false)} />
			)}

			<Input
				className={cn('nx-pl-9', inputClassName)}
				ref={input}
				value={value}
				onChange={(e) => {
							...
							<Fragment key={id}>
								...
								<li
									className={cn(
										...
										i === active
											? 'nx-bg-slate-100 nx-text-slate-900 contrast-more:nx-border-primary-500'
											: 'nx-text-slate-900 contrast-more:nx-border-transparent dark:nx-text-gray-300'
									)}
								>
									<Anchor
...
const searchIcon = (
	<svg
		...
		viewBox="0 0 16 16"
		fill="none"
		className="nx-absolute nx-left-3"
	>
		<path
